...
chpasswd:
  ...
  expire: False
write_files:
- path: /etc/network/interfaces.d/10-ens4.cfg
  owner: root:root
  permissions: '0644'
  content: |
    auto ens4
...
user_data_secret | sudo k3s kubectl apply -n metal3 -f -

sudo k3s kubectl patch baremetalhost ${BMHOST} -n metal3 --type merge \
